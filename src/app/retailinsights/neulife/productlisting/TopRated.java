...
public class TopRated extends ListActivity {

	ProgressDialog pd;
	private static final String METHOD = "topRated";
	private final String SOAP_ACTION = SessionStorage.webserviceNamespace
			+ "service.php/topRated";
	private final String SOAP_ACTION_FLAVOUR = SessionStorage.webserviceNamespace
			+ "service.php/getFlavours";
	...
	HashMap<String, String> mapFlavFlavCode;
	ArrayList<HashMap<String, String>> alFlavFlavCode;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (!cd.isConnectingToInternet()) {
			...
			InternetConnection add = new InternetConnection(TopRated.this);
			add.show();
		...
		viewButton1.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				//Toast.makeText(getApplicationContext(), "size: "+alFlavourSelected.size()+""+alFlavourSelected.get(0), Toast.LENGTH_LONG).show();
				...
				Log.w("alProducts size in TopRated: "+alProducts.size(), "");
				db = new DatabaseHandler(getApplicationContext());
				db.resetTablesSort();
				for (int i = 0; i < alProducts.size(); i++) {
					Log.w("alProductListing.get(i).getProductId(): "+alProducts.get(i).getProductId(), "");
					db.addProductsSort(
						alProducts.get(i).getProductId(),
				...
				gridView.setAdapter(new ProductListingAdapter(TopRated.this,
						alProducts));
				listView.setAdapter(new ProductListingListviewAdapter(
						TopRated.this, alProducts));
					...
					case 0:
						if(tvNoneApplied.getText().toString().equals(""))
						{
							...
							gridView.setAdapter(new ProductListingAdapter(
									TopRated.this, hmProductsOnPrice));
		...
		listview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (listFlag == 1) {
					...
					String textValue = parent.getAdapter().getItem(position)
							.toString();
					Toast.makeText(getApplicationContext(), "price: "+textValue,
					...
					String textValue = parent.getAdapter().getItem(position)
							.toString();
					db = new DatabaseHandler(TopRated.this);
					db.getFlavourCode(textValue);
					//listviewPos.add(position);
					alFlavourSelected.add(db.getFlavourCode(textValue));
					//alFlavourSelected.add();
					Toast.makeText(getApplicationContext(), "flavCode: "+db.getFlavourCode(textValue),
							Toast.LENGTH_SHORT).show();
					alSubCategory.add(db.getFlavourCode(textValue));
				}
			}
		});
		...
		lvSubCategories.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				case 2:
					listFlag = 3;
					db = new DatabaseHandler(getApplicationContext());
					arrayList = db.getFlavor();
					flavours = db.getFlavorString();
					setListAdapter(new ArrayAdapter<String>(
							getApplicationContext(),
	...
	public void getTopRated() throws JSONException {
		...
		try {
			...
			Log.d("Product Details toprated", response.toString());
			JSONObject json = new JSONObject(response.toString());
			JSONArray jsonarray = json.getJSONArray("toprated");
	...
	public void getFlavors1() throws JSONException {
		...
		// bodyOut is the body object to be sent out with this envelope
		envelope.bodyOut = request;
		// request.addProperty("categoryid", "60");
		HttpTransportSE transport = new HttpTransportSE(
				SessionStorage.webserviceURL);
		try {
			transport.call(SOAP_ACTION_FLAVOUR, envelope);
			Object response = envelope.getResponse();
			Log.d("Flavors toprated", response.toString());
			JSONObject json = new JSONObject(response.toString());
			JSONArray jsonarray = json.getJSONArray("Flavours");
			flavCode = new String[jsonarray.length()];
			flavName = new String[jsonarray.length()];
			alFlavFlavCode = new ArrayList<HashMap<String,String>>();
			for (int i = 0; i < jsonarray.length(); i++) {
				JSONObject object = jsonarray.getJSONObject(i);
				flavCode[i] = object.getString("id");
				flavName[i] = object.getString("name");
				mapFlavFlavCode = new HashMap<String, String>();
				mapFlavFlavCode.put(object.getString("name"), object.getString("id"));   
				alFlavFlavCode.add(mapFlavFlavCode);
			}
	...
	private class AsyncCallWS_Flavors extends AsyncTask<String, Void, Void> {

		@Override
		protected void onPreExecute() {
			// TODO Auto-generated method stub
			super.onPreExecute();
			pd = new ProgressDialog(TopRated.this);
		...
		protected void onPostExecute(Void result) {
			...
			pd.dismiss();
			db = new DatabaseHandler(TopRated.this);
			for (int i = 0; i < alFlavFlavCode.size(); i++) {
				// addFlavor(String flavCode, String flavor)
				db.addFlavor(flavCode[i], flavName[i]);
				/*flavCode[i] = object.getString("id"); 
				flavName[i] = object.getString("name");*/
			}
		}
	...
	private class AsyncCallWS extends AsyncTask<String, Void, Void> {
		...
		protected void onPostExecute(Void result) {
			...
			if (alProductListing == null) {
				...
				listView.setAdapter(new ProductListingListviewAdapter(
						TopRated.this, alProductListing));
			...
			AsyncCallWS_Flavors asyncCallWS_Flavors = new AsyncCallWS_Flavors();
			asyncCallWS_Flavors.execute();
		}
